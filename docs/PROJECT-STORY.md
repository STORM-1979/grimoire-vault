...
## Часть VIII · UX-волна (2026-05-07)

После того как функциональный костяк (vaults, auth, search, kanban,
attachments, push, telegram-bot) был на месте, накопилась стопка
мелких UX-неровностей: общий шаблон формы для категорий с разными
потребностями, native селекты, ломающиеся при тёмной теме, отсутствие
сортировки на категории, неоптимальные размеры картинок при загрузке.
Эта волна — **полировка пер-категорийного опыта** без миграций
схемы, по принципу «каждая категория получает столько уникального
UX, сколько действительно нужна, и ни больше».

### Микро-стратегия

Вместо разовых hot-fix'ов — выработали несколько переиспользуемых
паттернов:

1. **`ThemedSelect`** — единый стилизованный дропдаун.  Реплейсит
   нативный `<select>` везде, где он попадается.  Один компонент, шесть
   мест применения сразу.
2. **`metadata` JSONB как escape hatch** — все per-категорийные поля
   (Vercel/Git/DB ссылки, prompt model, extraLinks, creds) живут в
   `entries.metadata`, без новых колонок в схеме.  Миграции схемы —
   только когда поле универсально-полезно для большинства категорий.
3. **Optimistic local state + realtime debounce** — для кейсов где
   серверный апдейт каскадирует (kanban reorder), локальное состояние
   доверяется на короткое окно, realtime эхо игнорируется.
4. **localStorage для UX-настроек** — sort mode пер-категория,
   custom kanban columns, portfolio site URL.  То, что не критично к
   cross-device parity, но важно для повседневного UX, не лезет в БД.
5. **Pure-client image pipeline** — Canvas API без зависимостей.
   Картинки сжимаются в браузере перед R2-аплоадом, оригинал нигде
   не сохраняется.

### Что сделалось end-to-end

Каждая категория получила лицо:

- **Designs / Active Projects** — auto-fill из URL (og:meta + Microlink
  screenshot fallback).
- **Ideas** — плиточная сетка вместо плоского списка.
- **Prompts** — copy-the-prompt на ховере, заголовок-первый порядок,
  themed select модели.
- **Skills** — приём полных shell-команд в поле URL, с извлечением
  ссылки внутри для og:fetch.
- **Kanban** — кастомные колонки + переименование дефолтов + анти-
  телепорт-фикс + edit-модалка.
- **Active Projects** (бывшая Portfolio) — полный мини-CRM проекта:
  Vercel/Git/DB ссылки в карточке, ТЗ + extraLinks + creds на детальной.

И сквозные улучшения:

- **Сортировка** в каждой категории + фильтр по тегам как dropdown +
  чипы тегов.
- **Сжатие картинок** на клиенте (50–70 % экономии на типичной
  фотке без визуальной деградации).
- **Перевод на русский** при extraction og:meta — английские GitHub-
  страницы, туториалы и т.д. сразу читаются по-русски.
- **Hover-copy** для категорий со свободно-форматным URL.

### Lesson learned

Самые полезные UX-апдейты — **не новые фичи**, а починка
«невидимых трений»:
- 409 «уже есть» который теперь self-heals вместо ошибки в banner'е
- Карточки канбана которые перестали телепортироваться при drop
- HTML5 `type="url"` который ругался на наши `/api/r2/...` пути
- Service worker bumps чтобы старый бандл не залипал на открытых
  вкладках

Каждый такой фикс по отдельности — мелочь.  Совокупно — приложение
перестаёт раздражать при ежедневном использовании, что в personal-
проекте важнее любой новой функциональности.

---

## Часть IX · Долгий взгляд
