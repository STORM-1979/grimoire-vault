...
function MediaCardImpl({ item, big, selected, bulkSelected, onBulkToggle, onTogglePin, onDelete, onEdit }: MediaCardProps) {
  ...
  return (
    <div data-entry-id={item.id} onClick={onClick} className="cat-card group block cursor-pointer">
      ...
      <div className="mt-2">
        <h4 className="font-display text-[14px] font-medium leading-tight text-ivory group-hover:text-emerald-200 transition line-clamp-2">
          {item.title}
        </h4>
        {item.description && (
          <p className="text-[11.5px] text-ivory-dim mt-1 leading-snug font-light line-clamp-2">{item.description}</p>
        )}
